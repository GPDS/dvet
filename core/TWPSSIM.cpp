...
void TPWSSIM::computeTPVQI(TemporalMap RefTempMap, TemporalMap TestTempMap, Video RefVideo, Video TestVideo, Video RefVideo0, Video TestVideo0, Sobel RefSobel,int row, int col)
{
	...
	for(N = 0; N < Noframes - 1; N++)
	{
		...
		for(register int i = 0; i < Nolines; i += b)
			for(register int j = 0; j < Nocols; j += b)
			{
				for(register int k = i; k < (i+b); k++)
					for(register int n = j; n < (j+b); n++)
					{
			...
			// TODO : Perguntar por que o 8
				for(register int k = i; k < (i+8); k++)
					for(register int n = j; n < (j+8); n++)
					{
